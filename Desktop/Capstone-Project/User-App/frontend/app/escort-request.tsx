    ...
    if (state === 'waiting') {
      interval = setInterval(async () => {
        try {
          ...
          if (response.data && response.data.status === 'assigned') {
            setOfficer({
              name: response.data.officer_name || 'Officer',
              photo: response.data.officer_photo,
            });
            setEta(response.data.estimated_wait || 5);
            setState('assigned');
          }
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await escortAPI.create({
        ...
        destination_lat: destinationCoords?.lat || 45.0880,
        destination_lng: destinationCoords?.lng || -64.3670,
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          ...
          {/* Destination — tap map to pick */}
          <View style={styles.field}>
            <Text style={styles.label}>Tap map to pick destination</Text>
            <CampusMap
              style={{ height: 180, borderRadius: 12, marginBottom: SPACING.sm }}
              showUserLocation
              onMapPress={(coord) =>
                setDestinationCoords({ lat: coord.latitude, lng: coord.longitude })
              }
              locations={
                destinationCoords
                  ? [{ id: 'dest', lat: destinationCoords.lat, lng: destinationCoords.lng, name: 'Destination', color: COLORS.accent }]
                  : []
              }
            />

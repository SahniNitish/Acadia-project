...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateUser = async (data: Partial<User>) => {
    // Update Firebase display fields
    if (firebaseUser) {
    ...
    // Persist to campus backend
    try {
      await authAPI.updateProfile(data);
    } catch (err) {
      console.log('Backend profile update error:', err);
    }
  ...
  const refreshUser = async () => {
    // Pull from campus backend as source of truth
    try {
      ...
      console.log('refreshUser backend error, falling back to Firebase:', err);
      if (firebaseUser) {

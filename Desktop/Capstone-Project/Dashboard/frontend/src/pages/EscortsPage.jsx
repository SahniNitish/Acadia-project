...
const SortableEscortCard = ({ escort, onAction, formatTimeAgo }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">{escort.studentPhone || 'No phone'}</p>
          </div>
        </div>
...
const KanbanColumn = ({ title, status, escorts, color, onAction, formatTimeAgo }) => {
...
export default function EscortsPage() {
  ...
  const handleStatusChange = async (escortId, newStatus, startWalk = false) => {
    try {
      ...
      toast.success(`Escort ${newStatus === 'completed' ? 'completed' : 'updated'}`);
    } catch (error) {
  ...
  const handleDragEnd = async (event) => {
    ...
    // This is a simplified version - in production you'd have drop zones for each column
  };
                      ...
                      <td>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">{escort.studentPhone}</p>
                          </div>
                        </div>
                      </td>
                      <td>
